<div class="box">
	<div class="box-header">
		Form input Data Customer
	</div>
	<div class="box-body">
		<form action=" <?php echo base_url() ?>index.php/welcome/Simpancustomer" method="POST">
		Nama <input type="text" name="nama" class="form-control">
		Alamat <input type="text" name="alamat" class="form-control">
		Jenis Kelamin <input type="text" name="jk" class="form-control">
		Nomor Handphone <input type="text" name="no_telepon" class="form-control">
		Keterangan <input type="text" name="keterangan" class="form-control">
		<br>
		<button class="btn btn-primary btn-md pull-right fa fa-check" type="submit"> SIMPAN</button>
	</div>
</div>
<div class="box">
	<div class="box-header">
		<h4>DATA CUSTOMER</h4>
	</div>
	<div class="box-body">
		<table class="table table-bordered table-hover" id="example1">
			<thead>
				<td>ID Customer</td>
				<td>Nama </td>
				<td>Alamat</td>
				<td>Jenis Kelamin</td>
				<td>Nomor Handphone</td>
				<td>Keterangan</td>
				<td>Aksi</td>
			</thead>
			<?php
			foreach ($data_customer as $key => $tampilkan) {
			 	echo "<tr>";
			 		echo "<td>$tampilkan->id_customer</td>";
			 		echo "<td>$tampilkan->nama</td>";
			 		echo "<td>$tampilkan->alamat</td>";
			 		echo "<td>$tampilkan->jk</td>";
			 		echo "<td>$tampilkan->no_telepon</td>";
			 		echo "<td>$tampilkan->keterangan</td>";
			 		echo "<td><button class='btn btn-warning btn-xs glyphicon glyphicon-pencil'> EDIT</button> <button 	class='btn btn-danger btn-xs glyphicon glyphicon-erase'> HAPUS</button></td>";
			 		echo "</tr>";
			 } 
			?>
		</table>
	</div>
</div>